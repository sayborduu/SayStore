...
struct InstallPreviewView: View {
	...
	@AppStorage("SayStore.installationMethod") private var _installationMethod: Int = 0
	@AppStorage("SayStore.serverMethod") private var _serverMethod: Int = 0
	...
	init(app: AppInfoPresentable, isSharing: Bool = false) {
		...
		let viewModel = InstallerStatusViewModel(isIdevice: UserDefaults.standard.integer(forKey: "SayStore.installationMethod") == 1)
	...
	private func _install() {
		Task { @MainActor in
			...
			let useNovaDNSDynamic = UserDefaults.standard.bool(forKey: "SayStore.useNovaDNSDynamic")
