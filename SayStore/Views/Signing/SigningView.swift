...
struct SigningView: View {
	...
	init(app: AppInfoPresentable) {
		self.app = app
		let storedCert = UserDefaults.standard.integer(forKey: "SayStore.selectedCert")
			...
			if let error {
				...
				if _temporaryOptions.post_installAppAfterSigned {
					DispatchQueue.main.asyncAfter(deadline: .now() + 0.4) {
						NotificationCenter.default.post(name: Notification.Name("SayStore.installApp"), object: nil)
					}
				}
				dismiss()
			}
