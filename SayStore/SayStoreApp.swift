...
struct SayStoreApp: App {
	...
	var body: some Scene {
		WindowGroup {
			...
			// dear god help me
			.onAppear {
				if let style = UIUserInterfaceStyle(rawValue: UserDefaults.standard.integer(forKey: "SayStore.userInterfaceStyle")) {
	...
	private func _handleURL(_ url: URL) {
		if url.scheme == "SayStore" {
			/// SayStore://import-certificate?p12=<base64>&mobileprovision=<base64>&password=<base64>
			...
			/// SayStore://export-certificate?callback_template=<template>
...
class AppDelegate: NSObject, UIApplicationDelegate {
	...
	private func _createPipeline() {
		...
		let pipeline = ImagePipeline {
			...
			let dataCache = try? DataCache(name: "dev.alexbadi.SayStore.datacache") // disk cache
	...
	private func _addDefaultCertificates() {
		guard
			UserDefaults.standard.bool(forKey: "SayStore.didImportDefaultCertificates") == false,
		...
		do {
			...
			UserDefaults.standard.set(true, forKey: "SayStore.didImportDefaultCertificates")
		} catch {
